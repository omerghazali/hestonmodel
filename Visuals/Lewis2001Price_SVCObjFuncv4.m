function [modelPrice, error] = Lewis2001Price_SVCObjFuncv4(marketPrice,bidAskWeights,T,K, u, w, S, r, q, params)
%%
kappa = params(1);
theta = params(2);
sigma = params(3);
...
for t=1:length(T)
    for x = 1:length(u) %calc P1 integrand for each maturity at absciccas
        Integrand1(x) = Lewis2001Integrand_P1(u(x), kappa,theta,rho,sigma,T(t),S,r,q,v0);
    end
    for k = 1:length(K) % for each different strike within split tables
        for x = 1:length(u)
            Integrand2(x) = w(x)*Lewis2001Integrand_P2(u(x), T(t),S,K(k),r,q,Integrand1(x));
