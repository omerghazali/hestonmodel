function finalError  = Lewis_DoubleHeston_SVCObjFun(marketPrice,bidAskWeights,T,K, u, w, S, r, q, params)
...
for t=1:length(T)
    for x = 1:length(u) %calc P1 integrand for each maturity at absciccas
        Integrand1(x) = Lewis_DoubleHestonCF_P1(u(x),kappa1, theta1, sigma1,v01, rho1, kappa2, theta2, sigma2, v02, rho2,T(t),S,r,q);
    end
    for k = 1:length(K) % for each different strike within split tables
        for x = 1:length(u)
            Integrand2(x) = w(x)*Lewis_DoubleHestonCF_P2(u(x), T(t),S,K(k),r,q,Integrand1(x));
