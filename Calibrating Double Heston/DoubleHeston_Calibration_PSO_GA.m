...
for rep = 1:repeat
    ...
    for limit = 1:length(genLimits)
        ...
        localPSStart = tic;
        [PSParams,PSError] = particleswarm(@(params) Lewis_DoubleHeston_SVCObjFun(marketPrice,bidAskWeights,T,K, u32, w32, S, r, q, params),10,lb2,ub2,optionsPS);

        %%  Local from PSO start with higher resolution
        [localPSParams, localPSError] = fmincon(@(params) Lewis_DoubleHeston_SVCObjFun(marketPrice,bidAskWeights,T,K, u128, w128, S, r, q, params),PSParams,[],[],[],[],lb2,ub2,[],optionsLocal);
        ...
        localGAStart = tic;
        [GAParams,GAError] = ga(@(params) Lewis_DoubleHeston_SVCObjFun(marketPrice,bidAskWeights,T,K, u32, w32, S, r, q, params),10,[],[],[],[],lb2,ub2,[],optionsGA);

        %% Local from GA start with higher resolution
        [localGAParams, localGAError] = fmincon(@(params) Lewis_DoubleHeston_SVCObjFun(marketPrice,bidAskWeights,T,K, u128, w128, S, r, q, params),GAParams,[],[],[],[],lb2,ub2,[],optionsLocal);
